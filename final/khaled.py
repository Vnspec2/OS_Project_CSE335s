...
def next_scr():
    ...
    for i in range(process_no_scale.get()):
            ...
            aindex=Scale(frame_label1,from_=0, to=12 ,resolution=0.1,length=100)   #ab: arrival, burst
            aindex.grid(row=i+1,column=1)
            at1.append(aindex)
            bindex = Scale(frame_label1, from_=0, to=12, resolution=0.1, length=100)  # ab: arrival, burst
